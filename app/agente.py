...
PRECEDENCIA DE FAMILIA: una palabra de familia/producto propia (KARCHER, BUSHING, TEE, CAMLOCK, PASAMURO, UNION, TUERCA, TAPON, etc.) DEFINE el tipo y GANA sobre un hilo suelto — un "M22"/"JIC" dentro de un KARCHER o BUSHING NO lo vuelve espiga métrica ni ADAP. Primero identifica la familia; recién después la medida. "karcher"/"conexion karcher"/"conección karcher"/"coneccion karcher" = tipo="CONEXION KARCHER" (familia propia, NO espiga métrica): su M22 es el hilo fijo, la medida es la manguera (1/4, 5/16, 3/8). Ej: "S/M Coneccion Karcher antiguo plastico M22*1/4" → tipo="CONEXION KARCHER", medida="1/4".
Métricas / DIN (rosca milimétrica): si la línea menciona "DIN", "métrica"/"milimétrica"/"mm", "TUB"/"tubo", o un hilo tipo "Mnn"/"DIN nn" (y NO es una familia propia como KARCHER), es una ESPIGA MÉTRICA ("TERM"/"terminal" = espiga). tipo = "ESPIGA HEMBRA MM LIVIANA" o "ESPIGA HEMBRA MM PESADA" (liviana/L/light → LIVIANA; pesada/H/heavy → PESADA; HEMBRA por defecto, MACHO solo si lo dice). medida = el hilo como "Mnn" ("DIN 22" o "M22" → "M22"). tubo = número de "TUB nn"/"tubo nn" → "nn". medidas = [manguera] (el "x 1/2"). angulo normal (90/45). NO conviertas el hilo ni el tubo a pulgadas.
...
PRECEDENCIA DE FAMILIA: una palabra de familia/producto propia (KARCHER, BUSHING, TEE, CAMLOCK, PASAMURO, UNION, TUERCA, TAPON, etc.) DEFINE el tipo y GANA sobre un hilo suelto — un "M22"/"JIC" dentro de un KARCHER o BUSHING NO lo vuelve espiga métrica ni ADAP. "karcher"/"conexion karcher"/"conección karcher"/"coneccion karcher" = tipo="CONEXION KARCHER" (familia propia, NO espiga métrica): el M22 es su hilo fijo, la medida es la manguera (1/4, 5/16, 3/8). Ej: "S/M Coneccion Karcher antiguo plastico M22*1/4" → tipo="CONEXION KARCHER", medida="1/4".
Espigas métricas (LIVIANA = métrica liviana, PESADA = métrica pesada — son tipos de rosca, NO marcas; solo si NO es una familia propia como KARCHER):
