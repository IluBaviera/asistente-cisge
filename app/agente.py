...
def _corregir_adaptador_ocr(parsed_list: list[dict]) -> list[dict]:
    ...
    for item in parsed_list:
        ...
        if tipo_up.startswith("ADAP") and "MACHO" not in tipo_up and "HEMBRA" not in tipo_up:
            nuevo = re.sub(rf'\b({_ROSCAS_RE})\b', r'MACHO \1', item["tipo"], flags=re.IGNORECASE)
            if nuevo != item["tipo"]:
                item["tipo"] = nuevo
                tipo_up = nuevo.upper()
                logger.info(f"_corregir_adaptador_ocr macho-default: '{linea[:60]}' → {nuevo}")
...
Abreviaciones de género y rosca (pedidos cortos): género 'm'=Macho, 'f'=Hembra (female, del inglés), 'h'=Hembra. Inicial de rosca: 'j'=JIC, 'n'=NPT, 'b'=BSP, 'o'=ORFS. Pueden venir pegadas género+rosca: 'mj'=Macho JIC, 'fj'=Hembra JIC, 'mn'=Macho NPT, 'fn'=Hembra NPT. Palabra completa: 'MNPT' (y 'MNT', error OCR frecuente que come la P) = Macho NPT; 'FNPT' (y 'FNT') = Hembra NPT. 'bushing' (o 'bushin'/'busing') = familia propia del catálogo BUSHING (NO es ADAP): tipo="BUSHING MACHO {rosca1} X HEMBRA {rosca2}" con MACHO primero. Los bushings suelen ser misma rosca en ambos lados (JIC×JIC, NPT×NPT). Ej: "bushing 20 mj -16 fj = 5" → tipo="BUSHING MACHO JIC X HEMBRA JIC", medidas=["1 1/4","1"], cantidad=5. Ej: "bushing mn 8 - fn 6" → tipo="BUSHING MACHO NPT X HEMBRA NPT", medidas=["1/2","3/8"].
Métricas / DIN (rosca milimétrica): si la línea menciona "DIN", "métrica"/"milimétrica"/"mm", "TUB"/"tubo", o un hilo tipo "Mnn"/"DIN nn", es una ESPIGA MÉTRICA ("TERM"/"terminal" = espiga). tipo = "ESPIGA HEMBRA MM LIVIANA" o "ESPIGA HEMBRA MM PESADA" (liviana/L/light → LIVIANA; pesada/H/heavy → PESADA; HEMBRA por defecto, MACHO solo si lo dice). medida = el hilo como "Mnn" ("DIN 22" o "M22" → "M22"). tubo = número de "TUB nn"/"tubo nn" → "nn". medidas = [manguera] (el "x 1/2"). angulo normal (90/45). NO conviertas el hilo ni el tubo a pulgadas.
Ej: "TERM 90 DIN 22 TUB 15 X 1/2 liviana" → tipo="ESPIGA HEMBRA MM LIVIANA", angulo="90", medida="M22", tubo="15", medidas=["1/2"].
Ej: "espiga 90 pesada tubo 14 1/2" → tipo="ESPIGA HEMBRA MM PESADA", angulo="90", tubo="14", medidas=["1/2"].
Aliases de marcas: JDE=JDEFLEX, VITI=VITILLO, MACTU=MACTUBI
Aliases de tipos: casco/casquillo = FERRULA | gir/girat = GIRATORIO | hex = HEXAGONAL | red/reductor = REDUCTOR | forx/orx = ORFS | bssp = BSP (typo frecuente) | bspp = BSPP | bspt = BSPT | silicona recta = MANG SILICONA RECTA | silicona codo 90 = MANG SILICONA CODO 90 | silicona codo 45 = MANG SILICONA CODO 45 | silicona codo = MANG SILICONA CODO | silicona corrugada = MANG SILICONA CORRUGADA | silicona radiador = MANG SILICONA RADIADOR | silicona = MANG SILICONA | pu/poliuretano = MANG PU | tapon macho milimetrico = TAPON MACHO METRICO | karcher / conexion karcher / conección karcher = CONEXION KARCHER (familia propia; su M22 NO la vuelve espiga métrica; medida = manguera 1/4, 5/16, 3/8) | union hembra npt = UNION HEMBRA NPT (NO es ESPIGA — es un accesorio adaptador con dos bocas hembra NPT; puede tener dos medidas distintas si es reducción) | esp h a p / espiga h a p / "a/plano" = ESPIGA HEMBRA ORFS A/P (asiento plano ORFS). "G" en abreviación de espiga (ej: H.G.A.P, Esp.H.G.A.P) = doble_hex=true (G=giratoria=doble hexágono). Ej: "Esp. H.A.P 90° -10" → tipo="ESPIGA HEMBRA ORFS A/P", angulo="90", medida="5/8". Ej: "Esp. H.G.A.P 90° -10-10" → tipo="ESPIGA HEMBRA ORFS A/P", angulo="90", doble_hex=true, medidas=["5/8","5/8"].
...
Abreviaciones de género y rosca: género 'm'=Macho, 'f'=Hembra (female), 'h'=Hembra. Inicial de rosca: 'j'=JIC, 'n'=NPT, 'b'=BSP, 'o'=ORFS. Pegadas género+rosca: 'mj'=Macho JIC, 'fj'=Hembra JIC, 'mn'=Macho NPT, 'fn'=Hembra NPT. Palabra completa: 'MNPT' (y 'MNT', error OCR que come la P) = Macho NPT; 'FNPT' (y 'FNT') = Hembra NPT. Ej: "CODO BAJA PRESION MNPT 8 x ESPIGA 10 = 5" → tipo="ESPIGA MACHO NPT", medidas=["1/2","5/8"], cantidad=5.
bushing / bushin / busing = familia propia BUSHING (NO es ADAP): tipo="BUSHING MACHO <rosca1> X HEMBRA <rosca2>", MACHO primero. Misma rosca ambos lados salvo que se indique. Ej: "bushing 20 mj -16 fj = 5" → tipo="BUSHING MACHO JIC X HEMBRA JIC", medidas=["1 1/4","1"], cantidad=5.
karcher / conexion karcher / conección karcher / coneccion karcher = familia propia CONEXION KARCHER (NO es espiga métrica aunque diga M22): el M22 es su hilo fijo; la medida es la manguera (1/4, 5/16, 3/8). Ej: "S/M Coneccion Karcher antiguo plastico M22*1/4" → tipo="CONEXION KARCHER", medida="1/4".
...
- Un solo tipo de rosca mencionado (o ninguno): es ESPIGA. El segundo lado hereda el mismo tipo; NO supongas JIC ni otro.
  ...
  Ej: "JIC16-JIC12"               → tipo="ADAP MACHO JIC X HEMBRA JIC", medidas=["1","3/4"]  ← ADAPTADOR (rosca mencio­nada 2 veces)
...
- Si el tipo de rosca aparece DOS VECES (aunque sea el mismo estándar) y no hay keywords "terminal"/"esp": es ADAPTADOR.
  IMPORTANTE: "H. JIC ... M. JIC" = JIC mencionado DOS veces = ADAPTADOR (H. y M. son género, no cambian que JIC aparece dos veces).
  Género en el tipo: si ambos lados son M./Macho → "ADAP MACHO X X MACHO Y"; si ambos son H./Hembra → "ADAP HEMBRA X X HEMBRA Y"; si difieren → "ADAP MACHO X1 X HEMBRA X2" (MACHO siempre primero, sin importar el orden en el texto).
  ...
  Ej: "bushing 20 mj -16 fj = 5"              → tipo="BUSHING MACHO JIC X HEMBRA JIC", medidas=["1 1/4","1"], cantidad=5  ← mj=Macho JIC, fj=Hembra JIC, bushing=familia propia BUSHING (no ADAP)
  ...
  "M." / "M " / "Macho" / "m" antes de rosca = género Macho. "H." / "Ho." / "H " / "Hembra" / "h" / "f" = género Hembra ("Ho." es error OCR frecuente de "H."; "f" = female). Pegado a la inicial de rosca: mj/fj/mn/fn (m/f/h género + j=JIC, n=NPT, b=BSP, o=ORFS).
